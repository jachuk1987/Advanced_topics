  ...
  return (
    <Paper elevation={3} style={{ padding: 16, marginBottom: 24 }}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField select label="Status" fullWidth value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as JobApplication['status'] })}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField label="Date Applied" type="date" fullWidth InputLabelProps={{ shrink: true }} value={form.dateApplied} onChange={(e) => setForm({ ...form, dateApplied: e.target.value })} />
        </Grid>
        <Grid item xs={12}>
